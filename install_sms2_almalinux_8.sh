...
#=========================================================================================================
# Program: install_sms2_almalinux_8.sh
#
# Ver         Date            Author          Comment    
# =======     ===========     ===========     ==========================================
# V1.0.00     2023-04-12      DW              Install SMS 2.0 on AlmaLinux 8.x by using Nginx as web server, 
#                                             and RabbitMQ as queue server.
# V1.0.01     2023-07-01      DW              1. Add OpenVZ platform checking to decide different installation 
#                                                action, due to OpenVZ doesn't support firewalld.
#                                             2. Avoid to use snap but ordinary packages for certbot installation,
#                                                since AlmaLinux 8.x handles snapd in a way SMS 2.0 installation
#                                                can't cooperate. 
# V1.0.02     2023-10-10      DW              Add two more NPM libraries 'arraybuffer-encoding' and 
#                                             'hash-wasm'. Force upgrade NPM library 'qs' to version 6.5.3 or above
...
# V1.0.13     2025-04-22      DW              Replace NPM package 'telegram-bot-api' by 'telegramsjs'.
...
v1=`hostnamectl | grep "AlmaLinux 8" | wc -l`
if [[ "$v1" -eq 0 ]] 
then
...
#-- Note: AlmaLinux 8.x handles snapd installation by a weird way, so that it can't use snap for SMS 2.0 installation. --#
#--       Therefore, certbot packages, which are not from snap, are used here.                                         --#
echo "Install SSL certificates issuing and renew utility"
dnf -y install python2-tools python2-devel gcc python2-virtualenv augeas-libs libffi-devel openssl-devel python3-virtualenv >> /tmp/install.log
...
echo "Please write it down and change the passwords as once."
echo ""
